...
export async function GET(req: Request) {
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { id: payload.id },
      select: { role: true, hr: { select: {company: true}}, admin: { select: {company: true} } }
    })
    ...
    // Fetch all farmers of 1 company
    const users = await prisma.user.findMany({
      where: {
        role: UserRole.FARMER,
        farmer: {
          company: user.hr?.company || user.admin?.company
        }
      },
    ...
    worksheet.columns = [
      ...
      { header: 'Культура', key: 'crop', width: 15 },
